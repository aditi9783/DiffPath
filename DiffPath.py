...
def mymainstuff( expf, pathwaysdir, outputdir = '.' ):
    # expf = sys.argv[1]; # expression file
    # pathwaysdir = "./KEGGpathways"
    ...
    exp_obj = geneexp(expf, genenameidx, foldchangeidx, state1idx, state2idx, outputdir);
    ...
    allkgmlfiles = [f for f in listdir(pathwaysdir)]
    ...
    for f in allkgmlfiles:
        ...
        pobj = pathway(pathwaysdir + "/" + f);
        print "\n===\n", f, pobj.pathname;
        #    print "genedict: ", pobj.genedict;
        pgenes = expLabelsPathgenes(pobj.genedict, genelabels);  # pgenes has node id as key and exp label as value
        ...
        for i in range(0, nrand):
            ...
            gdbrand = graphs();
            randpg = randomizeLabels(pgenes, labelprob);  # randomize gene exp labels for genes in this pathway
    ...
    plt.hist(pvals);
    plt.savefig(outputdir + "/pval_hist.pdf");
    plt.close();

    plt.hist(pvals_corr);
    plt.savefig(outputdir + "/pval_corr_hist.pdf");
    plt.close();
...
def expLabelsPathgenes( pgenedict, genelabels ):
...
def randomizeLabels( pg, labelprob ): # read a dict with key: node is, and value: exp label, and return new dict with randomized labels
